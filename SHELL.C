...
WORD find_shell_button( WORD button );
void init_shell_logo();
void update_shell_logo();
...
void goto_shell_buttons( WORD button )
{
UWORD *types_list;
WORD index;
...
void modify_current_item_r( WORD total_items )
{
...
Rdb_gadget *rdb;
	...
	shell->lt_indicator_sprite->textman=0;
	shell->rt_indicator_sprite->textman=0;
...
reloop:
	if ( shell->master_option_active == YES )
		...
		if ( (cjoy->db & (PAD_X|PAD_SQU|PAD_STA|PAD_TRI)) && (shell->timer==0) )
			...
			if ( (cjoy->db & PAD_TRI) && ( shell->keyboard_active == NO ) )
				{
				if ( (find_shell_button( SHELL_PREV )) || (find_shell_button( SHELL_QUIT )) )
					{
					shell->timer=20;
					shell->current_button=0;
					}
...
//					if ( type != SHELL_QUIT )
		...
		{
		if ( cjoy->db & (PAD_X|PAD_SQU|PAD_TRI) )
			{
		...
		if ( (cjoy->db & PAD_TRI) && ( shell->keyboard_active==NO) )
			{
			if ( find_shell_button( SHELL_PREV ) )
				{
				goto_shell_buttons( SHELL_PREV );
				shell->current_button = SHELL_PREV;
				goto  reloop;
				}
			else
				{
				if ( find_shell_button( SHELL_QUIT ) )
					{
					goto_shell_buttons( SHELL_QUIT );
...
WORD xp;
	...
	shell->lolly_mode = LOLLY_DYING;
	shell->force_master_option_active_off = NO;
	...
	set_textman_slot( MAIN_TSLOT, 96 );		
	set_textman_slot( HILITE_TSLOT, 6 );		
...
/************************************************
 *															   *
 * Pass default button as							   *
 * SHELL_NEXT, SHELL_PREV etc or -1 for default *
 *															   *
 ************************************************/
...
//***** Count num of buttons in list ***** 
	...
	if ( (shell->last_button == 0) || ( shell->last_button == SHELL_QUIT ) )
...
/*** Last button was never set, so look for NEXT ***/
	...
	if ( shell->force_master_option_active_off )
		{
		shell->force_master_option_active_off = NO;
		shell->master_option_active = NO;
		shell->current_item = default_button;
		}
...
WORD div;
	...
	if ( sprite->textman == 0 )
		...
		sprite->x.w.hi += (sprite->x2.w.hi - sprite->x.w.hi)/6;
		...
		sprite->x.w.hi = sprite->x2.w.hi;
		}


	sprite->tint.r = sprite->tint.g = sprite->tint.b = ((timer/8)&1)?128:230;
	...
	if ( sprite->type2 == 0 )
		...
		/** Left **/
		FntPrint( "text=%s\n",ts->string );
		ts2=add_text_string ( tslot_poses[HILITE_TSLOT]+0 , sprite->textman->string , sprite->textman->font_type, milford_font_logic, ts->x, ts->y-8, ts->justify_type, -2, 0, 0, 10,0 );
		ts2->depth = ts->depth-1;
		ts2->text_color.r=(ts->text_color.r/div)+20;
		ts2->text_color.g=ts->text_color.g/div;
		ts2->text_color.b=ts->text_color.b/div;
		}
	else
		{
		/** Right **/
		ts2=add_text_string ( tslot_poses[HILITE_TSLOT]+1 , sprite->textman->string , sprite->textman->font_type, milford_font_logic, ts->x, ts->y+8, ts->justify_type, -2, 0, 0, 10,0 );
		ts2->depth = ts->depth-1;
		ts2->text_color.r=(ts->text_color.r/div)+20;
		ts2->text_color.g=ts->text_color.g/div;
		ts2->text_color.b=ts->text_color.b/div;
...
WORD dy;
	...
	sprite->xscale=sprite->yscale=256;
	...
	if ( shell->drop_buttons == NO )
		...
		dy = ( sprite->y.w.hi - shell->button_dest_y )/2;
		sprite->y.w.hi -= dy; 
...
//			shell->buttons_dropping=NO;	// button have stopped
			...
			{
			dy = (( sprite->y.w.hi - shell->button_dest_y )/2)+((sprite->group_num-SHELL_ITEM_BASE)*4) + 6 ;
			dy = (( sprite->y.w.hi - shell->button_dest_y )/2)+((sprite->group_num-SHELL_ITEM_BASE)*6) + 10 ;
			sprite->y.w.hi += dy;
			}
		}
...
//		if ( sprite->y.w.hi > (NTSC?400:1200) )
//			shell->buttons_dropping=NO;	// button have stopped

	/** Bottom lolly text **/
		if ( (shell->requester_box_active == NO) && ( shell->mode == FRONTEND ) )
			{
			ts=add_text_string ( tslot_poses[LOLLY_TSLOT]+2 , &shell->lolly_text_buffer[0][0] , TITLE_FONT, milford_font_logic, shell->bot_bar_text_x, shell->bot_bar[0]->y.w.hi, FONT_XC, -2, 0, 0,0,0 );
			ts->depth = NEAREST_DEPTH-1;
			ts=add_text_string ( tslot_poses[LOLLY_TSLOT]+3 , &shell->lolly_text_buffer[0][0] , TITLE_FONT, milford_font_logic_sub, shell->bot_bar_text_x-6, shell->bot_bar[0]->y.w.hi+12, FONT_XC, -2, 0, 0,0,0 );
...
//	ts = add_text_string ( tslot_poses[LOLLY_TSLOT]+0 , &shell->lolly_text_buffer[1][0] , MILFORD_FONT, milford_font_logic, shell->lolly_text_dest_x[0], -472, FONT_NORMAL, -2, 0, 0,0,0 );
...
//	shad = (Sprite *)shell->logo->prev;
	...
	if ( shell->mode == FRONTEND )
		...
		shell->logo->display = YES;
		shell->logo->frame = 0;
		shell->logo->x.w.hi = 0-((256*2)/2);
		shell->logo->y.w.hi = -(350+(22*4));

		shad->x.w.hi = shell->logo->x.w.hi-(8*2);
		shad->y.w.hi = shell->logo->y.w.hi+(8*4);
		shad->display = YES;
		shad->transp_rate = 2;
#endif
		}
	...
	if ( shell->mode != FRONTEND )
		...
		ts=add_text_string ( tslot_poses[LOLLY_TSLOT]+0 , &shell->lolly_text_buffer[1][0] , TITLE_FONT, milford_font_logic, -60, -472, FONT_RJUST, -2, 0, 0,0,0 );
		ts->depth = NEAREST_DEPTH+0;
		ts=add_text_string ( tslot_poses[LOLLY_TSLOT]+1 , &shell->lolly_text_buffer[1][0] , TITLE_FONT, milford_font_logic_sub, -60-6, -472+12, FONT_RJUST, -2, 0, 0,0,0 );
		ts->depth = NEAREST_DEPTH+1;
		}
...
void set_current_item( WORD num )
{
	shell->old_item = shell->current_item;
	shell->current_item = num;
	sfxSpot( HIT1, 30 );
}

void restore_current_item()
{
WORD old;

	old = shell->current_item;
...
{
WORD cnt;
BYTE logo_frames[]={ 0, 8,9,10,11,12 };
BYTE logo_transps[]={ -1,1,1,1,-1,-1 };
BYTE logo_brights[]={ 128,100,100,64,128,128 };

	return;
	shell->logo[0]=spawn_sprite ( TEST );
	return;


	for( cnt=0;cnt<6;cnt++)
		{
		shell->logo[ cnt ] = spawn_sprite( LOGO );

		sprite->use_frame_offs = YES;
		sprite->transp_rate = logo_transps[ cnt ];
		sprite->frame = logo_frames[ cnt ];
		sprite->depth = MIDDLE_DEPTH - cnt;
		sprite->x.w.hi = 0-((256*2)/2);
		sprite->y.w.hi = -(350+(22*4));
		sprite->tint.r = sprite->tint.g = sprite->tint.b = logo_brights[cnt];
		}

	add_process( update_shell_logo );

	shell->logo_shad = spawn_sprite( SHELSHD );
	sprite->frame = 1;
	sprite->x.w.hi = 0-((256*2)/2) - 16;
	sprite->y.w.hi = -(350+(22*4)) + 32;
	sprite->tint.r = sprite->tint.g = sprite->tint.b = 44;
	sprite->transp_rate = 2;
	sprite->depth = MIDDLE_DEPTH+1;
...
/** Init rage logo **/
	...
	shell->rage_logo = spawn_sprite( RAGELOGO );
	sprite->x.w.hi = 0-((72*2)/2);
	sprite->y.w.hi = 300;
	sprite->depth = MIDDLE_DEPTH;

	shell->rage_shad = spawn_sprite( SHELSHD );
	sprite->x.w.hi = 0-((72*2)/2) - 16;
	sprite->y.w.hi = 300+16;
	sprite->frame = 2;
	sprite->depth = MIDDLE_DEPTH+1;
	sprite->transp_rate = 2;
	sprite->tint.r = sprite->tint.g = sprite->tint.b = 44;
...
WORD cnt;
	...
	return;
	shell->logo[0]->frame = (timer/1)%8;	/** Ball goes 0-7 **/


	if ( shell->mode == FRONTEND )
		{
		shell->rage_logo->display = NO;
		shell->rage_shad->display = NO;
		for(cnt=0;cnt<6;cnt++)
			{
			shell->logo[cnt]->xscale = 128;
			shell->logo[cnt]->yscale = 128;
			shell->logo[cnt]->centre_if_scaled=YES;
			}
		}
	else
		{
		shell->rage_logo->display = YES;
		shell->rage_shad->display = YES;
		for(cnt=0;cnt<6;cnt++)
			{
			shell->logo[cnt]->xscale = 256;
			shell->logo[cnt]->yscale = 256;
			}
