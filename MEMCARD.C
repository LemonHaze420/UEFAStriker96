...
BYTE *get_expanded_file_name( BYTE *text );
...
WORD cnt;
	...
	set_rdb_gadget( 0, &mc_rdb[0], -242, 512, 242*2, 532 ,  -1 );
...
//			text_buffer = get_card_port_text();
			...
			if ( multitaps_connected_list[0] )
				sprintf( mc_info_buffer, mc_no_lt_card_present_text_tap[ config->language ], get_card_port_text() );
			else
...
//		if ( (share->game_type != CONFIG) ||( (share->game_type==CONFIG)&&(mc->total_specific_game_files[0])==0 ) )
		if ( (share->game_type != CONFIG) ||( (share->game_type==CONFIG)&&((mc->total_specific_game_files[0])==0 )&&(mc->total_blocks[0]==15) && (mc->total_game_files[0]) ) )
...
//						shell->current_item = 512+total_text_lines-1;
		...
		if ( ((cjoy->db & (PAD_X|PAD_SQU ))&&(mc->mode)) || ((share->game_type == CONFIG)&&(mc->card_present[0])&&(mc->total_specific_game_files[0]) )  || ((share->game_type == CONFIG)&&(mc->card_present[0])&&(mc->total_specific_game_files[0]==0) && ( mc->total_blocks[0]<15))  )
			{
			sfxSpot( BUTVOC, 30 );

			if ( (((shell->current_item-512) >= mc->total_game_files[ mc->selected_card])) || ((share->game_type == CONFIG)&&(mc->total_specific_game_files[0]) )  || ((share->game_type == CONFIG)&&(mc->total_specific_game_files[0]==0) && ( mc->total_blocks[0]<15))  )
				{
...
//				else
				...
				if ( share->game_type == CONFIG )
					...
					if ( mc->total_specific_game_files[0] )
						{
				...
				if ( mc->result == McErrNone )
					...
					text = get_expanded_file_name( &mc->filename[len] );
					sprintf( text_buffer, mc_save_ok_text[config->language], text );
					mc_rdb[1].size.y = 512;
				...
				if ( strncmp( &mc->filename[len], mc->save_appendix, app_len ) == 0 )
					...
					sprintf( text_buffer,  mc_are_you_sure_overwrite_text[config->language], get_expanded_file_name(&mc->filename[ len ]) );
					mc_rdb[0].size.y = 512;
					mc_rdb[5].size.y = 512;
					mc_rdb[1].size.y = 512;
					...
					strcpy( text_buffer2, get_expanded_file_name(&newname[ len ]) );
					
					sprintf( text_buffer,  mc_are_you_sure_overwrite_with_text[config->language], get_expanded_file_name(&mc->filename[ len ]), text_buffer2 );
					result = do_request( text_buffer, mc_yes_no_butts, -128, SHELL_NO );
				...
				if ( result == SHELL_YES )
					...
					if ( mc->result == McErrNone )
						...
						text = get_expanded_file_name( &mc->filename[len] );
						if ( newfile == NO )
							sprintf( text_buffer, mc_overwrite_ok_text[config->language], text );
						else
							sprintf( text_buffer, mc_save_ok_text[config->language], text );
...
WORD cnt;
	...
	set_rdb_gadget( 0, &mc_rdb[0], -242, 512, 242*2, 428+32,  -1 );
	set_rdb_gadget( 0, &mc_rdb[1], -1, 512, -1, -1, 0 );
	set_rdb_gadget( 0, &mc_rdb[4], -1, 512, -1, -1, -1 );	// box behind info box
...
BYTE *text;
	...
	if ( (cjoy->db & (PAD_X|PAD_SQU )) && (mc->mode) )
		...
		if ( mc->result == McErrNone )
			...
			text = get_expanded_file_name( &mc->filename[len] );
			sprintf( text_buffer, mc_load_ok_text[config->language], text );
			do_request( text_buffer, mc_ok_butt, -128, SHELL_OK );
			shell->current_button = SHELL_OK;
			shell->module_abort=YES;
			}
...
WORD ret = Mc_ok;
	...
	if ( (mc->card_present[0] || mc->card_present[1]) )
		...
		if ( (mc->total_specific_game_files[0]==0) && (mc->total_specific_game_files[1]==0) )
			...
			sprintf( text_buffer, mc_no_specific_game_files_text[ config->language ], game_types_text[ config->language ][ share->game_type ] );
			mc_rdb[0].size.y = 512;
			mc_rdb[5].size.y = 512;
			do_request( text_buffer , mc_ok_butt, -128, SHELL_OK );
			ret = Mc_cancel;
	...
	if ( mc->card_present[ mc->selected_card ] && mc->directory_loaded[ mc->selected_card ] )
		...
		for( cnt=0; cnt< mc->total_files[ mc->selected_card ]; cnt++ )
			...
			if ( strncmp ( dir_name, full_name, len ) == 0 )
				...
				text = get_expanded_file_name( text );	// only view striker games of specific type eg League games

				mc_rdb[0].size.y = -208-32-32-32-16-16-4;

				add_text_string ( tslot_poses[MAIN_TSLOT]+32+cnt, text , TINY_FONT, milford_font_logic, 0, -(90*4)+(36*yp), FONT_XC, yp+512, 0, 0,0,0 );
				mc_file_nums[yp] = cnt;
				yp++;
				}
		...
		if ( load == NO )
			...
			if ( (share->game_type != CONFIG) || ((share->game_type == CONFIG) && ( mc->total_specific_game_files[ mc->selected_card ] == 0 )) )
				...
				if ( free_slots )
					{
					mc_rdb[0].size.y = -208-32-32-32-16-16-4;
					text = mc_free_slot_text[ (share->game_type == CONFIG)?1:0][ config->language ];
					add_text_string ( tslot_poses[MAIN_TSLOT]+32+cnt, text , TINY_FONT, milford_font_logic, 0, -(90*4)+(36*yp), FONT_XC, yp+512, 0, 0,0,0 );
					}
				}
			}
...
WORD card;
	...
	if ( mc_info_buffer[0] )
		{
		ts = add_text_string ( tslot_poses[MAIN_TSLOT]+30, mc_info_buffer , MILFORD_FONT, milford_font_logic, 0, 0, FONT_YC+FONT_XC, -2, 0, 0,0,0 );
...
BYTE *only_txt;
	...
	/**** Slot num *****/
	ts = add_text_string ( tslot_poses[MAIN_TSLOT]+64, text_buffer , MILFORD_FONT, milford_font_logic, 0, 232-48, FONT_XC, 1, 0, 0,0,0 );
...
BYTE exp_name_string[ 64 ];

BYTE *get_expanded_file_name( BYTE *text )
{
WORD len, cnt;

	for( cnt=0; cnt< TOTAL_GAME_TYPES; cnt++ )
		{
		len = strlen( memory_card_appendix_names[ cnt ] );

		if ( strncmp( memory_card_appendix_names[ cnt ], text, len )==0 )
			{
			text+=len;

			if ( (cnt == CONFIG) || (cnt == FRIENDLY_SAVE) )
				sprintf(exp_name_string, "%s", game_types_text[ config->language ][ cnt ] );
			else
				sprintf(exp_name_string, "%s #%s", game_types_text[ config->language ][ cnt ], text );
			return ( exp_name_string );
			}
		}

#if DEBUG
	display_error( "get expanded fname string not found\n");
#endif
