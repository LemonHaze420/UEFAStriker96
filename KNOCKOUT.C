...
WORD list[]={3,2,1,0,0};
	...
	if ( share->game_type == KNOCKOUT )
		...
		if ( share->game_stat == GAME_OVER ) //game_over )
			{
			set_shell_bottom_buttons( knockout_over_shell_buttons, -1 );	
			...
			if ( share->game_stat != GAME_IN_PROGRESS )
				set_shell_bottom_buttons( knockout_stat_shell_buttons ,-1 );	
			else
				{
				//if ( share->ko_data.draws_to_do  )
				//	set_shell_bottom_buttons( knockout_quit_next_shell_buttons,-1 );	
				//else
					set_shell_bottom_buttons( knockout_end_of_round_shell_buttons,-1 );	
				}
			}
		}
	else
		...
		set_shell_bottom_buttons( euro2000_qual_shell_buttons,-1 );														 
		}
	...
	ko_windx_adjust = comp->ko_data.current_round;
...
//	FntPrint("cr=%d winpos=%d drawsleft=%d\n",comp->ko_data.current_round, comp->ko_data.winning_list_pos, comp->ko_data.draws_to_do );
...
//		ko_windx = 1097;
	...
	for( x=0; x<5; x++ )
		...
		for( y=0; y<ylines; y++ )
			...
			if ( x >= ko_windx_base )
				{
				team_num = comp->ko_data.teams[ x ][ y ];
				if ( team_num != -1 )
					{
				...
				ts=add_text_string ( tslot_poses[MAIN_TSLOT]+cnt2, text_buffer, TINY_FONT, milford_font_logic, (-102*4)+(x*Ko_boxwd)-ko_windx-28, ((-94+(y*10))*4)+yoffs, FONT_XC, -2, 0, 10,0,fade_cnt/4 );
				fade_cnt++;
...
//					}
				...
				if ( (x == comp->ko_data.current_round) && ( share->game_stat != GAME_OVER ) )
					{
					if ( (share->team1 == comp->ko_data.teams[ x ][ y ] ) || (share->team2 == comp->ko_data.teams[ x ][ y ] )  )
						{
						if ( (timer/8)&1)
							{
							color.r >>= 1;
