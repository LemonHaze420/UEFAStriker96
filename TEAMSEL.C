...
#define FLAG_Y_SIZE (28*4)	//Y space between flags 

#define FLAG_X_CNT 4
#define FLAG_Y_CNT 6	// visible amount of flags on screen
...
BYTE ts_teams_index_list[ TOTAL_TEAMS ];


WORD diff_limit_lo, diff_limit_hi;

BYTE *ts_source_team_list;
...
RECT flag_clip_rect={ 0, (FLAGS_Y/4)+128, 512,  ((FLAG_Y_SIZE/4)*5)-2 };
...
WORD ycnt,mod,lines,teams;
	...
	if ( (share->game_type == FRIENDLY) || ( share->game_type == TERRITORY ) )
		{
		if ( share->club_comp == NO )
			{
			diff_limit_lo = COUNTRY_TEAMS_START;
			diff_limit_hi = COUNTRY_TEAMS_START + MAX_COUNTRY_TEAMS-1;
			}
		else
			{
			diff_limit_lo = CLUB_TEAMS_START;
			diff_limit_hi = CLUB_TEAMS_START + MAX_CLUB_TEAMS-1;
			}
		}
	else
		{
		if ( (share->game_type == LEAGUE) || (share->game_type == KNOCKOUT) )
			{
		/*** League and knockout have difficulty settings ***/
			...
			if ( share->club_comp )
				...
				if ( share->game_difficulty == 0 )
					{
					diff_limit_lo = CLUB_TEAMS_START;
					diff_limit_hi = CLUB_TEAMS_START+10;
					}
				if ( share->game_difficulty == 1 )
					{
					diff_limit_lo = CLUB_TEAMS_START+11;
					diff_limit_hi = CLUB_TEAMS_START+20;
					}
				if ( share->game_difficulty == 2 )
					{
					diff_limit_lo = CLUB_TEAMS_START+21;
					diff_limit_hi = CLUB_TEAMS_START+MAX_CLUB_TEAMS-1;
					}
				}
			else
				...
				if ( share->game_difficulty == 0 )
					{
					diff_limit_lo = COUNTRY_TEAMS_START;
					diff_limit_hi = COUNTRY_TEAMS_START+20;
					}
				if ( share->game_difficulty == 1 )
					{
					diff_limit_lo = COUNTRY_TEAMS_START+21;
					diff_limit_hi = COUNTRY_TEAMS_START+40;
					}
				if ( share->game_difficulty == 2 )
					{
					diff_limit_lo = COUNTRY_TEAMS_START+41;
					diff_limit_hi = COUNTRY_TEAMS_START+MAX_COUNTRY_TEAMS-1;
					}
				}
			}
...
/*** Calculate no. of rows of flags from no. of teams selection ***/
	...
	if ( share->club_comp == NO )
		...
		border_flag = spawn_sprite( SHIRTH  );
	...
	for ( cnt=0;cnt<FLAG_X_CNT * ts_visible_spr_ycnt; cnt++ )
		{
		/*** Spawn flag ***/
		ts_sprs[cnt] = spawn_sprite ( FLAG00A+cnt );
		sprite->display=YES;
		sprite->group_num = ts_group;

		/*** Spawn drop shadow ***/

		if ( share->club_comp == YES )
			ts_drop_sprs[cnt] = spawn_sprite ( SHIRTSH );
		else
	...
	shell->force_master_option_active_off=YES;
...
WORD clicks;
WORD ts_cx_store, ts_cy_store;

	ts_cx_store = ts_dest_cx;
	ts_cy_store = ts_dest_cy;
...
/*** Set cursor X pos ***/
	...
	cnt = ((ts_dest_cy/FLAG_Y_SIZE) * FLAG_WINDX_CNT) + ( ts_dest_cx/FLAG_X_SIZE);

	if ( cnt >= share->teams_selection_size )
		{
		ts_cx = ts_dest_cx = ts_cx_store;
		ts_cy = ts_dest_cy = ts_cy_store;
		goto_shell_buttons( -1 );
		return;
		}


	border_flag->x.w.hi = ts_cx + FLAGS_X - 8;
	border_flag->y.w.hi = ts_cy - ts_windy + FLAGS_Y - 12;

	if ( share->club_comp )
		{
		border_flag->x.w.hi += 2 + 14;
		border_flag->y.w.hi += 8;
		}

	border_flag->tint.r = border_flag->tint.g = border_flag->tint.b = ((timer/8)&1)?110:220;
...
//	team = team_index_lists[ config->language ][ team ];
	...
	if ( (team >= USER_TEAMS_START) && (team < (USER_TEAMS_START+MAX_USER_TEAMS)) )
		{
		// team name is user-defined...

		txt = config->team_detail[ team-USER_TEAMS_START ].name;
		...
		if ( team >= CLUB_TEAMS_START )
			{
			/** Club teams are all in the same language **/
			txt = team_names[0][team];
			}
		else
	...
	for ( cnt=0; cnt<share->teams_selection_size; cnt++ )
		...
		if ( humcpu != 0  )
			...
			if ( share->game_type == FRIENDLY )
				sprintf( text_buffer, "%s",  team_name_ptr );
			else
				sprintf( text_buffer, "%s (%s)",  team_name_ptr, humcpu_txt[config->language][ humcpu-1 ] );

			ts = add_text_string ( tslot_poses[MAIN_TSLOT]+y , text_buffer, TINY_FONT, milford_font_logic, 270*1, -(76*4)+(y*TS_SCROLL_YS)-ts_scroll_y, FONT_XC, -2, 0, 20,0,fade_delay );
...
//	ts_team_num = team_index_lists[ config->language ][ ts_team_num ];
		...
		add_text_string ( tslot_poses[MAIN_TSLOT]+16 , ts_team_name_ptr , MILFORD_FONT, milford_font_logic, -228*1, -(89*4), FONT_XC, -2, 0, 0,0,0 );
		}
...
WORD ts_comp_cnt, ts_human_cnt, ts_diff_cnt, ts_tot_teams_cnt;
WORD ts_opts;
...
WORD cnt, val;
	...
	ts_comp_cnt=ts_human_cnt=ts_diff_cnt=ts_tot_teams_cnt=-1;

	ts_opts=0;
	if ( (share->game_type == FRIENDLY) || (share->game_type == LEAGUE) || (share->game_type == KNOCKOUT) )
		{
		ts_comp_cnt = ts_opts;
		ts_opts++;
		}
	...
	set_shell_bottom_buttons( snt_shell_buttons,-1 );	


	snt_yp = ((-(((ts_opts-1) * snt_ys) + ( snt_yht*ts_opts)))/2)-(16*4);


	for( cnt=0; cnt<ts_opts; cnt++ )
		{
		set_rdb_gadget( 0, &snt_rdb[cnt], -350, snt_yp + ( (snt_ys+snt_yht)*cnt)+(7*4), 350*2,snt_yht, cnt );
		}
...
WORD min,max, yline;
	...
	modify_current_item ( ts_opts );


	if ( shell->current_item == ts_tot_teams_cnt )
		{
		min=0;
		max=8;

	/** Set total teams ( league, knockout only ) **/
	...
	if ( shell->current_item == ts_human_cnt )
		{
	/** Set human teams ( all competitions ) **/
		...
		if ( (share->total_human_teams > 1) && (pads_connected < 2) )
			...
			{
			max = share->teams_in_competition;
			if ( share->total_human_teams > 8 )
				share->total_human_teams=min;	/** No more than 8 human teams **/
			}

		}

	if ( shell->current_item == ts_diff_cnt )
		{
	/** Set difficulty ( league, knockout only ) **/
	...
	if ( shell->current_item == ts_comp_cnt )
		modify_byte( &share->club_comp, 0, 1 );
...
/** Print human teams x ***/
	if ( ts_human_cnt >=0 )
		...
		add_text_string ( tslot_poses[MAIN_TSLOT]+ts_human_cnt , text_buffer, MILFORD_FONT, milford_font_logic, 0, snt_yp+(ts_human_cnt*(snt_ys+snt_yht)), FONT_XC, ts_human_cnt, 0, 8,0,2 );
		}

/*** Competition mode **/
	if ( ts_comp_cnt >= 0 )
		{
		sprintf( text_buffer, "%s : %s",comp_mode_text[config->language], club_int_text[config->language][share->club_comp] );
		ts=add_text_string ( tslot_poses[MAIN_TSLOT]+ts_comp_cnt ,text_buffer, MILFORD_FONT, milford_font_logic, 0, snt_yp+(ts_comp_cnt*(snt_ys+snt_yht)), FONT_XC, ts_comp_cnt, 0, 8,0,4 );
		}

/** Print total teams x ***/
	if ( ts_tot_teams_cnt >= 0 )
		{
		sprintf( text_buffer, "%s : %d",snt_totteams_txt[config->language],share->teams_in_competition );
		add_text_string ( tslot_poses[MAIN_TSLOT]+ts_tot_teams_cnt , text_buffer, MILFORD_FONT, milford_font_logic, 0, snt_yp+(ts_tot_teams_cnt*(snt_ys+snt_yht)), FONT_XC,ts_tot_teams_cnt , 0, 8,0,0 );
		}
...
/** Print difficulty ***/
	if ( ts_diff_cnt >= 0 )
		...
		add_text_string ( tslot_poses[MAIN_TSLOT]+ts_diff_cnt ,text_buffer, MILFORD_FONT, milford_font_logic, 0, snt_yp+(ts_diff_cnt*(snt_ys+snt_yht)), FONT_XC, ts_diff_cnt, 0, 8,0,4 );
		}
	...
	switch( ts_teams_index_list[ team_num ] )
		...
		case 1:
			if ( cpu_teams_to_generate )
				...
				{
				if ( (human_teams_to_generate) && (cpu_teams_to_generate) )
					{
					mode = 1;
					}
				}
...
void team_select()
{
	...
	if ( (share->game_type == FRIENDLY) || (share->game_type == LEAGUE) || (share->game_type == KNOCKOUT) )
		...
		if ( share->club_comp == NO )
			{
			ts_source_team_list = &ts_country_teams_list[ config->language ][0];
			share->teams_selection_size = MAX_COUNTRY_TEAMS + MAX_USER_TEAMS;
			}
		else
			{
			ts_source_team_list = ts_club_teams_list;
			share->teams_selection_size = MAX_CLUB_TEAMS;
			}
...
void calc_teams_to_generate()
{
	...
	if ( (share->game_type == EURO2000) || (share->game_type == WORLDCUP)  )
		cpu_teams_to_generate = 0;

	if ( share->game_type == EUROFINAL )
		{
		cpu_teams_to_generate-=2;
		}
	return;
