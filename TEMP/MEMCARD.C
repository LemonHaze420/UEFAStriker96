...
BYTE *mc_no_card_present_text[]={
	"No Memory card present in Memory card slot %d",
};


BYTE *mc_unformat_text[]={
	"Memory card in Memory card slot %d is unformatted.\nFormat Memory card?",
	"Memory card in Memory card slot %d is unformatted.\nFormat Memory card?",
	"Memory card in Memory card slot %d is unformatted.\nFormat Memory card?",
	"Memory card in Memory card slot %d is unformatted.\nFormat Memory card?",
};
...
WORD cnt;
	...
	set_shell_bottom_buttons( memcard_ok_butt,0 );
...
Textman_struct *ts;
	...
	if ( (mc->card_present[0]==0) && (mc->card_present[1]==0 ) && (mc->card_busy[0]==NO && mc->card_busy[1]==NO) )
		{
		if ( share->game_type == CONFIG )
			strcpy( mc_info_buffer, mc_no_lt_card_present_text[ config->language ] );
		else
...
//		show_slot_num_text(); //was here
	...
	free_slots = (15 - mc->total_files[ mc->selected_card ])?1:0;
...
WORD ret = Mc_cancel;
	...
	if ( (mc->card_present[ mc->selected_card ] == YES) && (mc->card_busy[ mc->selected_card ] == NO ) )
		...
		if ( mc->card_formatted[ mc->selected_card ] == NO )
			...
			sprintf( text_buffer, mc_unformat_text[config->language], mc->selected_card+1 );
			mc_rdb[0].size.y = 512;
			mc_rdb[5].size.y = 512;
			result = do_request( text_buffer, mc_yes_no_butts, -128, SHELL_NO );
			if ( result == SHELL_YES )
...
//	FntPrint("total files %d, striker files %d\n",mc->total_files[ mc->selected_card ], mc->total_game_files[ mc->selected_card ] );
	...
	free_slots = 15 - mc->total_files[ mc->selected_card ];
...
//	mc->exit_func = mc_exit_func;
	...
	for(cnt=0;cnt<80;cnt++)
		...
		if ( (mc->command == Mc_ready) && (mc->card_busy[0]==NO) && (mc->card_busy[1]==NO ) && (mc->card_present[0] || mc->card_present[1] ) )
			{
			if ( mc->card_present[0] && mc->total_specific_game_files[0] )
				{
				mc->selected_card=0;
			 	do_mc_load_config();
				return;
				...
				{
				if ( mc->card_present[1] && mc->total_specific_game_files[1] )
					{
					mc->selected_card=1;
			 		do_mc_load_config();
					return;
...
WORD cnt;
	...
	for(cnt=0;cnt<80;cnt++)
		...
		if ( (mc->command == Mc_ready) && (mc->card_busy[0]==NO) && ( mc->total_specific_game_files[0] ) && (mc->card_present[0] ) )
			return YES;
		}
...
BYTE *only_txt;
	...
	if ( share->game_type == CONFIG )
		{
		sprintf( text_buffer, "%s %c %s\n",card_num_text[ config->language ], mc->selected_card?'B':'A', only_text[config->language] );
		}
	else
		sprintf( text_buffer, "%s %c\n",card_num_text[ config->language ], mc->selected_card?'B':'A' );
