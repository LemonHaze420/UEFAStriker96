...
LONG cnt;
	...
	//show_message("doing init frontend");
  	init_frontend();

	//show_message("doing run title");
	run_title();
...
{
	share->demo_mode = NO;
	//show_message("doing init sound");
	initsound( 127, 127 );

	//show_message("doing init process exec");
	init_process_executor();


	//show_message("doing init player edit boot");
	...
	while(abort_frontend == NO )
		...
		switch( share->thread_pos )
			...
			case 3:
				...
				if ( share->perform_draw == YES )
					{

					share->perform_draw = NO;
			...
			case 5:
				...
				if ( calculate_winner() )
					...
					share->thread_pos = 6;	 /* goto 6 when game has been played */
					share->perform_draw = YES; 
					}
				else
					{
	...
	while( abort_frontend == NO )
		...
		switch ( share->thread_pos )
			...
			case 5:
				...
				share->thread_pos = 6; // go to 6 after game played
				share->perform_draw = YES; 
			...
			/** Pc comes here after game played **/
				...
				if ( share->game_stat == QUIT_COMPETITION )
					...
					{
					share->game_stat = GAME_IN_PROGRESS;
					share->ko_data.page_mode = 0; // signal to knockout page that a game has just been played
					share->thread_pos = 3;
					ko_calc_results();
					share->perform_draw=YES;
		...
		switch( share->thread_pos )
			...
			case 3:
				if ( share->game_stat == GAME_IN_PROGRESS )
					{
					/*** Jump to here only after a game has been played ***/
				...
				if ( (share->game_stat == ROUND_OVER) && (share->territory_data.current_territory == 5 ) )
					{
					share->thread_pos = 0;	// back territory select
					}
			...
			case 4:
				if ( share->game_stat == ROUND_OVER )
					{
					if ( (share->territory_data.completed-1) <= share->territory_data.current_territory )
						{
						if ( check_human_territory_winner() )
							{
							/** Winning team was HUMAN, so goto next round **/
							share->territory_data.current_territory++; 	// Auto select next territory
							share->territory_data.completed = share->territory_data.current_territory+1; 				// This territory is over, make another available
							share->thread_pos = 0;
							}
						else
							{
			...
			case 6:
				if ( share->game_stat == QUIT_COMPETITION )
					...
					share->game_stat = GAME_IN_PROGRESS;	// now we are into a game
					calc_league_stats();
					share->thread_pos = 3; // back to stat screen
					}
			break;
	...
	while(abort_frontend == NO )
		...
		switch( share->thread_pos )
			...
			case 3:
				...
				if ( game_over == NO )
					{
					share->classic_data.completed[ share->classic_data.current_match ] = YES;

					//share->classic_data.current_match++;

					for(cnt=0;cnt<8;cnt++)
						{
						if ( share->classic_data.completed[ cnt ] == NO )
							break;
						}

					if ( cnt >= 4 )
						share->classic_data.available = cnt+1;
	...
	while(abort_frontend == NO )
		...
		switch( share->thread_pos )
			...
			case 6:
				...
				share->thread_pos = 7; // goto 6 when game has been played
				share->perform_draw = YES; 
...
//					printf("group=%d\n",share->current_group);
			...
			case 6:
				...
				share->thread_pos = 7; // goto 7 when game has been played
				share->perform_draw = YES; 
