...
//*** Clear name string ***
	...
	if ( src_name_string )
		...
		en_cur_char = strlen ( src_name_string );
		}
	...
	for ( y=0;y<EN_GRID_HT;y++ )
		...
		for ( x=0;x<EN_GRID_WD;x++ )
			...
			wd = (frame->real_w*SCREEN_UNIT_WD)/config->screen_xres;
...
//	FntPrint("cx=%d\n",en_name_cursor->x.w.hi );
	...
	if ( cjoy->db & (PAD_X|PAD_SQU|PAD_CIR|PAD_TRI)  )
		{
...
//	ts = entname_gad.textman_struct;
	ts = add_text_string ( tslot_poses[MAIN_TSLOT]+30, entname_namestring_ptr , MILFORD_FONT, milford_font_logic, EN_NAME_X, EN_NAME_Y, FONT_XC, -2, 0, 0,0 );
	...
	en_name_cursor->y.w.hi = EN_NAME_Y + (16*4); 
