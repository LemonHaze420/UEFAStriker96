...
//	y += SCREEN_UNIT_HT/2;
	...
	while ( 1 )
		...
		if ( fr < 242 )
			...
			sprite->x.w.hi = x;
			sprite->y.w.hi = y + (frame->yoffs*SCREEN_UNIT_HT)/config->screen_yres;
			...
			x +=  ((frame->real_w*SCREEN_UNIT_WD)/config->screen_xres ) + font->font_data->x_gap_size;



			temp = sprite->y.w.hi + ((frame->h*SCREEN_UNIT_HT)/config->screen_yres);
			...
			if ( (fr == FONT_CR ) || (fr == FONT_ENDSTRING) )
				...
				if ( textman->justify_type & FONT_XC )
					...
					while( line_sprite_cnt )
						...
						temp = sprite->x.w.hi  + ((frame->real_w*SCREEN_UNIT_WD)/config->screen_xres );
 ...
 *******************/
	...
	if ( sprite->sequence_num == FONT_INIT )
		...
		sprite->timer1 = ts->delay_until_fade_up;
	...
	if ( sprite->sequence_num == FONT_STAGE2 )
		...
		if ( ( ( ((flash_counter/spd)&1)==0) ) && ( ts->flash_text ) )
			sprite->brightness = ts->max_brightness/3;
		else
			sprite->brightness = ts->max_brightness;
	...
	if ( dim )
		{
		sprite->tint.r = ( ts->text_color.r * sprite->brightness ) >> 8;
		sprite->tint.g = ( ts->text_color.g * sprite->brightness ) >> 8;
		sprite->tint.b = ( ts->text_color.b * sprite->brightness ) >> 8;
		}
	else
		{
...
// check all strings with active=YES and to_die=YES
	...
	if ( text->active == YES )
		...
		if ( (my_strcmp ( string, text->string )) || ( font_type != text->font_type) )
			{
	...
	if ( spawn )
		...
		text->text_color.r = text->text_color.g = text->text_color.b = 128;
	...
	if ( (text->spawn_group == shell->current_item)  )
		{
		shell->lt_indicator_sprite->textman = text;
		shell->rt_indicator_sprite->textman = text;
		}
