...
//	ts_team_num = team_index_lists[ config->language ][ ts_team_num ];
	...
	add_text_string ( tslot_poses[MAIN_TSLOT]+16 , ts_team_name_ptr , MILFORD_FONT, milford_font_logic, -228*1, -(89*4), FONT_XC, -2, 0, 0,0 );
...
//			val = cnt;
		...
		set_rdb_gadget( 0, &snt_rdb[cnt], -250, snt_yp + ( snt_ys*cnt), 250*2,58, cnt );
		}
...
/** Print human teams x ***/
	...
	add_text_string ( tslot_poses[MAIN_TSLOT]+cnt , text_buffer, MILFORD_FONT, milford_font_logic, 0, snt_yp+(cnt*snt_ys)-20, FONT_XC, 1, 0, 32,0 );

/** Print difficulty ***/
	cnt=2;
	sprintf( text_buffer, "%s : %s",snt_diff_txt2[config->language], snt_diff_txt1[config->language][share->game_difficulty] );
	ts=add_text_string ( tslot_poses[MAIN_TSLOT]+cnt ,text_buffer, MILFORD_FONT, milford_font_logic, 0, snt_yp+(cnt*snt_ys)-20, FONT_XC, 2, 0, 32,0 );
	if ( (share->game_type == TERRITORY) || (share->game_type == EURO2000) || (share->game_type == EUROFINAL) || (share->game_type == WORLDCUP) || (share->game_type == FRIENDLY )    )
		{
		ts->text_color.r = ts->text_color.g = ts->text_color.b = 32;
		}
