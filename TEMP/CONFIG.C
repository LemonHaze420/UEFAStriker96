...
WORD cnt;
	...
	if ( share->config_loaded == NO )
		...
		set_title_lolly( 0, restore_config_lolly_text[config->language]  );
//		shell->mode = FRONTEND;
		...
		if ( check_load_config() == YES )
			...
			for(cnt=0;cnt<2;cnt++)
				{
				ts=add_text_string ( tslot_poses[REQUEST_TSLOT]+0, autoload_conf_file_txt[ config->language ] , MILFORD_FONT, milford_font_logic, 0, 0, FONT_XC+FONT_YC, -2, 0, 0,0 );
