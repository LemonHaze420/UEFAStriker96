...
/*** Shirt num ***/
	...
	if ( shell->current_item == 4 )
		modify_byte( &te->shirt_color1[ te_player_type ], 0, 14 ); 
	if ( shell->current_item == 5 )
		modify_byte( &te->shirt_color2[ te_player_type ], 0, 14 ); 

/** Change sleeves **/
	if ( shell->current_item == 6 )
		modify_byte( &te->sleeve_style[ te_player_type ], 0, TE_SLEEVES-1 ); 
	if ( shell->current_item == 7 )
		modify_byte( &te->sleeve_color1[ te_player_type ], 0, 14 ); 
	if ( shell->current_item == 8 )
		modify_byte( &te->sleeve_color2[ te_player_type ], 0, 14 ); 
...
/** Change shorts **/
	...
	if ( shell->current_item == 10 )
		modify_byte( &te->short_color1[ te_player_type ], 0, 14 ); 
	if ( shell->current_item == 11 )
		modify_byte( &te->short_color2[ te_player_type ], 0, 14 ); 

/** Change socks **/
	if ( shell->current_item == 12 )
		modify_byte( &te->sock_style[ te_player_type ], 0, TE_SOCKS-1 ); 
	if ( shell->current_item == 13 )
		modify_byte( &te->sock_color1[ te_player_type ], 0, 14 ); 
	if ( shell->current_item == 14 )
		modify_byte( &te->sock_color2[ te_player_type ], 0, 14 ); 
	...
	if ( (shell->keyboard_active == NO) )
		...
		add_text_string ( tslot_poses[MAIN_TSLOT]+30+y , text_buffer, MILFORD_FONT, milford_font_logic, (-70*4)/2, -(62*4) + (y*21*4)-(16*4)-TE_YM, FONT_XC, 0, 0, 128, 0,0 );

		y=1;
		add_text_string ( tslot_poses[MAIN_TSLOT]+30+y , pl_ed_button_names[config->language][0], MILFORD_FONT, milford_font_logic, (-84*4)/2, -(62*4) + (y*21*4)-(16*4)-TE_YM, FONT_XC, 1, 0, 128, 0,0 );

		y=2;
		add_text_string ( tslot_poses[MAIN_TSLOT]+30+y , pl_ed_player_words[ te_player_type ][ config->language ], MILFORD_FONT, milford_font_logic, (-84*4)/2, -(62*4) + (y*21*4)-(16*4)-TE_YM, FONT_XC, 2, 0, 128, 0,0 );

		y=3;
		sprintf( text_buffer, "%s #%d-", te_ed_shirt_txt[ config->language ],te->shirt_style[te_player_type]+1 );
		add_text_string ( tslot_poses[MAIN_TSLOT]+30+y , text_buffer, MILFORD_FONT, milford_font_logic, -652, -(62*4) + (y*21*4)-(16*4)-TE_YM, FONT_RJUST, 3, 0, 128, 0 ,0);

		y=4;
		sprintf( text_buffer, "%s #%d-", te_ed_sleeve_txt[ config->language ] ,te->sleeve_style[te_player_type]+1);
		add_text_string ( tslot_poses[MAIN_TSLOT]+30+y , text_buffer, MILFORD_FONT, milford_font_logic, -652, -(62*4) + (y*21*4)-(16*4)-TE_YM, FONT_RJUST, 6, 0, 128, 0,0 );

		y=5;
		sprintf( text_buffer, "%s #%d-", te_ed_shorts_txt[ config->language ],te->short_style[te_player_type]+1 );
		add_text_string ( tslot_poses[MAIN_TSLOT]+30+y , text_buffer, MILFORD_FONT, milford_font_logic, -652, -(62*4) + (y*21*4)-(16*4)-TE_YM, FONT_RJUST, 9, 0, 128, 0,0 );

		y=6;
		sprintf( text_buffer, "%s #%d-", te_ed_socks_txt[ config->language ],te->sock_style[te_player_type]+1 );
		add_text_string ( tslot_poses[MAIN_TSLOT]+30+y , text_buffer, MILFORD_FONT, milford_font_logic, -652, -(62*4) + (y*21*4)-(16*4)-TE_YM, FONT_RJUST, 12, 0, 128, 0,0 );
...
//** Sleeve cluts **
	...
	num=20;
	download_clut( (BYTE *)te_pals, TED_SHIRT_BACK, 32*num,208,32 );
...
void *process;
	...
	te_textures = allocate_mem( 1, MAX_TE_STRIP_TSIZE );	// alloc 126k
	unpropak( te_textures_pak, te_textures );
	
	pl_num_textures = allocate_mem( 1, MAX_TE_PL_NUMS_TSIZE );	// alloc 46k
	unpropak( pl_num_textures_pak, pl_num_textures );

	pl_textures = allocate_mem( 0,MAX_PL_FACE_TSIZE );	// alloc 120k
...
/***** Loop for team/player edit, called by title *****/
	while( 1 )
		...
		if ( call_enter_name )
			...
			{
			if ( shell->current_button == SHELL_PREV )
				break;
