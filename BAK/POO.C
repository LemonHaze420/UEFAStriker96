...
WORD cards_present;
	...
	mc_total_text_lines = mc->total_game_files[ mc->selected_card ];
	free_slots = (15 - mc->total_blocks[ mc->selected_card ])?1:0;

	if ( free_slots )
		mc_total_text_lines++;	/* Include 'create new game' text */
	...
	if ( cards_present == NO )
		...
		modify_current_item( mc_total_text_lines+1 );	

		if ( shell->current_item == mc_total_text_lines )
			{
		...
		if ( ( mc->card_present[ mc->selected_card ]==NO) || (mc->new_card[ 0 ] && mc->card_present[0]) || (mc->new_card[1] && mc->card_present[1]) )
			shell->current_item = mc_total_text_lines;


		if ( ((cjoy->db & (PAD_X|PAD_SQU ))) || ((share->game_type == CONFIG)&&(mc->card_present[0])&&(mc->total_specific_game_files[0]) )  )
			{
			sfxSpot( BUTVOC, 30 );

			if ( (shell->current_item==(mc_total_text_lines-1)) || ((share->game_type == CONFIG)&&(mc->total_specific_game_files[0]) ) )
				{
				...
				filenum = mc_file_nums[ shell->current_item ];	// get file number from ALL files
...
WORD cards_present;
	...
	mc_total_text_lines = mc->total_specific_game_files[ mc->selected_card ];
	...
	if ( mc->card_present[ mc->selected_card ] && mc->directory_loaded[ mc->selected_card ] )
		...
		for( cnt=0; cnt< mc->total_files[ mc->selected_card ]; cnt++ )
			...
			if ( strncmp ( dir_name, full_name, len ) == 0 )
				...
				add_text_string ( tslot_poses[MAIN_TSLOT]+32+cnt, text , TINY_FONT, milford_font_logic, 0, -(90*4)+(36*yp), FONT_XC, yp, 0, 0,0 );
				mc_file_nums[yp] = cnt;
				yp++;
				}
...
BYTE *only_txt;
	...
	/**** Slot num *****/
	ts = add_text_string ( tslot_poses[MAIN_TSLOT]+64, text_buffer , MILFORD_FONT, milford_font_logic, 0, 232-48, FONT_XC, mc_total_text_lines, 0, 0,0 );
