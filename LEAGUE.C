...
UWORD league_init_stat_buts[]= { SHELL_SAVE, SHELL_PREV, SHELL_NEXT, 0 };
...
WORD cnt, cnt2, group;
	...
	for( group=0; group<TOTAL_GROUPS; group++ )
		...
		for( cnt=0; cnt< TOTAL_LEAGUE_TEAMS; cnt++ )
			{
			comp->league_data[ group ][ cnt ].played 			= 
			comp->league_data[ group ][ cnt ].won					= 
			comp->league_data[ group ][ cnt ].drawn	 			= 
			comp->league_data[ group ][ cnt ].lost	 			= 
			comp->league_data[ group ][ cnt ].goals_for			= 
			comp->league_data[ group ][ cnt ].goals_against	= 
			comp->league_data[ group ][ cnt ].goal_diff			= 
			comp->league_data[ group ][ cnt ].points				= 0;
			}
		}
...
//	lstat_yp = -276 + yoffs;
	...
	switch ( share->game_type )
		{
		case TERRITORY:
			set_shell_bottom_buttons( territory_stat_buts,-1 );

			if ( share->game_stat == ROUND_OVER )
				{
				set_title_lolly( final_territory_stat_lolly_text[ comp->territory_data.current_territory ][config->language], game_types_text[ config->language ][ share->game_type ]  );
				}
			else
				{
				set_title_lolly( territory_stat_lolly_text[ comp->territory_data.current_territory ][config->language], game_types_text[ config->language ][ share->game_type ]  );
				}
		break;
		...
		case EURO2000:
			...
			if ( share->game_stat == GAME_OVER )
				...
				set_shell_bottom_buttons( league_stat_buts_end,-1 );
				...
				if ( share->game_stat == GAME_IN_PROGRESS )
					...
					set_shell_bottom_buttons( next_quit_buts,-1 );
					}
				else
					{
					set_shell_bottom_buttons( league_stat_buts,-1 );
		...
		default:
			...
			if ( share->game_stat == GAME_INIT )
				set_shell_bottom_buttons( league_init_stat_buts,-1 );
			else
				{
				if ( share->game_stat == GAME_OVER )
					set_shell_bottom_buttons( league_stat_buts_end,-1 );
...
//*** Print team name in left column ***
	for ( cnt=0; cnt<team_cnt; cnt++ )
		{
  		index_cnt = comp->league_data[ ls_group ][ cnt ].win_pos;
		...
		ts = add_text_string ( tslot_poses[MAIN_TSLOT]+text_cnt, text_buffer , MILFORD_FONT, milford_font_logic, -616, lstat_yp+(index_cnt*LSTAT_YSPACE)-0, FONT_RJUST, -2, 0, 8,0,index_cnt );
...
//*** Print 'played won drawn texts ***
	for ( x=0; x<8; x++ )
		{
		ts=add_text_string ( tslot_poses[MAIN_TSLOT]+text_cnt, lstat_txt[config->language][x] , MILFORD_FONT, milford_font_logic, (-15+xspace[x])*4, lstat_yp-92, FONT_XC, -2, 0, 8,0,0 );
...
//*** Print Each teams stats ***
	...
	for ( cnt=0; cnt<team_cnt; cnt++ )
		...
		for ( x=0; x<8; x++ )
			{
			if ( x == 0 ) sprintf( text_buffer, "%d",comp->league_data[ ls_group ][cnt].played );
			if ( x == 1 ) sprintf( text_buffer, "%d",comp->league_data[ ls_group ][cnt].won );
			if ( x == 2 ) sprintf( text_buffer, "%d",comp->league_data[ ls_group ][cnt].drawn );
			if ( x == 3 ) sprintf( text_buffer, "%d",comp->league_data[ ls_group ][cnt].lost );
			if ( x == 4 ) sprintf( text_buffer, "%d",comp->league_data[ ls_group ][cnt].goals_for );
			if ( x == 5 ) sprintf( text_buffer, "%d",comp->league_data[ ls_group ][cnt].goals_against );
			if ( x == 6 ) sprintf( text_buffer, "%d",comp->league_data[ ls_group ][cnt].goal_diff );
			if ( x == 7 ) sprintf( text_buffer, "%d",comp->league_data[ ls_group ][cnt].points );

			add_text_string ( tslot_poses[MAIN_TSLOT]+text_cnt, text_buffer , MILFORD_FONT, milford_font_logic, (-15+xspace[x])*4, lstat_yp+(index_cnt*LSTAT_YSPACE)-0, FONT_XC, -2, 0, 8,0,index_cnt+x );
	...
	if ( (share->game_type == EURO2000) || (share->game_type == EUROFINAL) || (share->game_type == WORLDCUP)  )
		...
		if ( share->game_stat == GAME_IN_PROGRESS ) 
			...
			add_text_string ( tslot_poses[MAIN_TSLOT]+text_cnt, text_buffer , MILFORD_FONT, milford_font_logic, 0, league_stat_rdb[2].y1-16, FONT_XC, -2, 0, 8,0,0 );
			text_cnt++;
			}
		else
			{
			...
			add_text_string ( tslot_poses[MAIN_TSLOT]+text_cnt, text_buffer , MILFORD_FONT, milford_font_logic, 0, league_stat_rdb[2].y1-16, FONT_XC, 0, 0, 8,0,0 );
			text_cnt++;
...
void init_euro2000_qual_results()
{
	if ( share->game_type == EURO2000 )
		...
		set_shell_bottom_buttons( euro_qual_teams_buts,-1 );
		}
	else
		{
		set_title_lolly( euro_finalist_teams_lolly_text[config->language] , game_types_text[ config->language ][ share->game_type ] );
		set_shell_bottom_buttons( euro_final_teams_buts,-1 );
		}
...
Textman_struct *ts;
	...
	if ( share->game_type == EURO2000 )
		{
		/** Group winner title ***/
		add_text_string ( tslot_poses[MAIN_TSLOT]+text_cnt, eq_group_winner_text[config->language] , MILFORD_FONT, milford_font_logic, 0, -328, FONT_XC, -2, 0, 8,0,0 );
		text_cnt++;

		/*** List of winners ****/
		for( cnt=0; cnt<9; cnt++ )
			{
			team = comp->euro_final_data.teams[ cnt ]; 
			text = get_team_name( team );
			ts = add_text_string ( tslot_poses[MAIN_TSLOT]+text_cnt, text , TINY_FONT, milford_font_logic, 0, -256+(cnt*40), FONT_XC, -2, 0, 8,0,0 );
...
//				ts->text_color.b = 0;
		...
		/*** Runner up title ***/
		add_text_string ( tslot_poses[MAIN_TSLOT]+text_cnt, eq_group_runner_text[config->language] , MILFORD_FONT, milford_font_logic, 0, 116, FONT_XC, -2, 0, 8,0,0 );
		text_cnt++;

		/*** Runner up team ***/
		cnt=10;
		team = comp->euro_final_data.teams[ 9 ];  
		text = get_team_name( team );
		ts = add_text_string ( tslot_poses[MAIN_TSLOT]+text_cnt, text , TINY_FONT, milford_font_logic, 0, -256+(10*4)+(cnt*40), FONT_XC, -2, 0, 8,0,0 );
		/** Human teams in yellow **/
		if ( share->cpu_teams_index_list[ team ] == NO )
			{
//			ts->text_color.r = ts->text_color.g = 180;
//			ts->text_color.b = 0;
	...
	if ( share->game_type == EURO2000_KO )
		...
		for( cnt=0; cnt<16; cnt++ )
			...
			team = comp->euro_final_data.teams[ cnt ];
			text = get_team_name( team );
			ts = add_text_string ( tslot_poses[MAIN_TSLOT]+text_cnt, text , TINY_FONT, milford_font_logic, 0, -384+(11*4)+(cnt*36), FONT_XC, -2, 0, 8,0,0 );
			/** Human teams in yellow **/
			if ( share->cpu_teams_index_list[ team ] == NO )
				{
//				ts->text_color.r = ts->text_color.g = 180;
...
/*** Check for existance of a human team through to Euro final ***/
	...
	//* Are any of the 8 runner ups human ?
		for( cnt=0; cnt<8; cnt++ )
			{
			team = comp->euro_final_data.runner_teams[ cnt ];
